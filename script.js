...
document.body.style.background =
linear-gradient(rgba(0,0,0,0.85), rgba(0,0,0,0.9)),   url('https://images.unsplash.com/photo-1581092160562-40aa08e78837?auto=format&fit=crop&w=1600&q=80');
}
...
if (isEditMode) {
container.innerHTML +=   <div style="margin-top:40px;">   <button onclick="addCategory()">➕ Add Category</button>   <button onclick="addProduct()">➕ Add Product</button>   </div>  ;
}
}
/* ================= BODY BACKGROUND ================= */
...
document.body.style.background =
linear-gradient(rgba(0,0,0,0.85), rgba(0,0,0,0.9)),   url('https://images.unsplash.com/photo-1581092580497-e0d23cbdf1dc?auto=format&fit=crop&w=1600&q=80');
}
...
hero.style.background =
linear-gradient(rgba(0,0,0,0.75), rgba(0,0,0,0.85)),   url('https://images.unsplash.com/photo-1581092160562-40aa08e78837?auto=format&fit=crop&w=1600&q=80');
}
...
;   }   if (   msg === "hi" ||   msg === "hello" ||   msg === "namaste"   ) {   return 
...
reply +=   ${p.image ?<img src="${p.image}" style="width:100%;border-radius:10px;margin-bottom:10px;">: ""}   <b>${p.name}</b><br>   ⭐ Rating: ${extractRating(p.reviews) || "Not rated"}<br>   ₹${p.price}<br><br>  ;
});
...
<div style="   position:fixed;   top:10px;   left:10px;   background:#d4af37;   color:#000;   padding:6px 12px;   border-radius:6px;   font-weight:bold;   z-index:9999;">   ✏ EDIT MODE   </div>
);
enableHeroControl();
}
